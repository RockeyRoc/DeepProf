"""Diagnosis Skill：从答题/提问/错误轨迹推断知识状态（DESIGNv0.6 §6.3 / §18.1）。

**诚实边界（重要）**：学情模型**尚未接入**。
按 D-6（§15）与 §18.1，BKT 掌握度、IRT 题目标定与后续知识追踪由欧阳文凯负责，
必须以统一 `LearnerModel` 接口（models/learner）输出
...
2. 本 Skill 改为调用 ``host.call_tool("learner_estimate", ...)``；
...
#: 数据组交付估计结果时应满足的字段（与 models/learner/estimate.py 一致）
...
class DiagnosisSkill:
    ...
    纯规则查询，不需要宽面：只回显收到的输入字段，便于交接时对齐契约。
    """
    ...
    async def invoke(self, input: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "skill": self.name,
            "note": _MISSING,
            "learner_id": str(input.get("learner_id") or ""),
            "concept": str(input.get("concept") or ""),
            "inputs_received": sorted(str(key) for key in input),
            "expected_output_schema": list(_EXPECTED_OUTPUT_SCHEMA),
            "estimate": None,
            "uncertainty": None,
        }