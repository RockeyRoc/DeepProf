...
export class SessionClient {
  ...
  async create(title = "学习会话"): Promise<string> {
    const accepted = await this.commands.send(this.commands.create("session.new", { title }));
  ...
  async send(sessionId: string, content: string): Promise<CommandAccepted> {
    return this.commands.send(this.commands.create("message.send", { content }, sessionId));
  }
