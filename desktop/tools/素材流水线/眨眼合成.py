...
3. **眼带右边界还是蹭到了蝴蝶结**（2026-09-20 晚实测，**尚未修，已知**）。
   卡死范围之后没再"撑满整张图"，但 `x26%~70%` 的右边界（1086 宽的图上 = x760）
   仍会框进**头侧蝴蝶结的深蓝描边**：实测有一块 83px 的深色连通域在
   `x[723,759] y[463,472]`，右边正好贴着带边界 x=759、上边贴着 y=463 —— 是被带子**截断**的，
   不是睫毛。后果是贴图矩形向上向右多盖了一块，**把取材图的蝴蝶结贴到基准图上**，
   在鲸鱼蝴蝶结的白肚皮那里留下一道很淡的"双眼皮/重影"。
   · **影响很小、且不是新引入的**：量过蝴蝶结区域的变化，旧版 7.7% vs 2026-09-20 晚新版 8.0%，
     整帧 27327 vs 27722 px，几乎一样。
   · **要根治**得把 x 上界收到约 740，但实测**右眼的右边界在 x=735** —— 只剩 5px 余量，
     换个姿势就可能把眼睛切掉。所以**没有就这么改**，留个记录在这儿：
     真要动，别只调常数，改成"丢掉贴着带边界的深色连通域"这类按形状判的规则更稳。
"""
import argparse
import os
import sys
...
# ⚠️ 这一句**不能省**（2026-09-20 晚补）：Windows 控制台默认 GBK，而这个脚本要打印
#    `⚠️`(U+26A0) —— 那是个**变体选择符序列**，GBK 编不出来，会抛 UnicodeEncodeError。
#    崩点在**存盘之前**，所以不会留半个残文件，但按文档原样跑就是**直接失败**。
#    姊妹脚本 `归一化帧.py` 一直有这句，这个漏了。
try:
