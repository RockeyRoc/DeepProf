...
BG_CORE = 246
#: 「可能和背景混在一起」的亮度下限。**泛洪只走过比它更亮的像素**，
#: 所以这个值同时是「描边的拦水能力」：
#:
#: ⚠️ 2026-09-20 从 188 提到 230 —— 原来的值把**白头发当背景删了**。
#:    新角色是大面积白发，而白发的描边很细，抗锯齿之后边缘亮度会超过 188，
#:    泛洪就从那条缝穿进头发内部，把整片头发判成"和边缘连通的浅色区域"删掉。
#:    症状：**走路时头部一片透明**，而源图（白底）里头发是好好的。
#:
#:    实测这四格图：背景 253~255，头发的高光也在 250 上下 —— 颜色上分不开，
#:    所以只能靠【描边拦住泛洪】。提到 230 之后，描边（含抗锯齿边缘）能挡住，
#:    而纯白背景照样走得通。
#:
#:    取 230 而不是更高，是留出余量：背景如果不是纯白（比如 240 的浅灰底），
#:    仍然要被判成背景；而 230 以下一律当"角色"，保守不出错。
BG_SOFT = 230
...
#:
#: ⚠️ 这个值**不能定小**。实测（新角色走路四帧）：角色自己的脸离主块有
#:    3.2 / 5.1 / 8.0 px —— 一开始定 3px，结果四帧里三帧的脸照样被删。
#:    而真正的残片（邻格漏进来的）在 24px 以外 —— 所以 20px 是安全的分界：
#:    保住所有紧贴角色的部件与装饰，仍然滤掉远处残片。
NEAR_MAIN_PX = 20
