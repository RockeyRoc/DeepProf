"""教学动作 → 能力绑定表（DESIGNv0.6 §4.4 / §6.2 / §6.3）。
...
绑定字段（解析逻辑见 runtime/capabilities.py）：
    ...
    选模板的键由 ``select`` 从决策取值（``${字段}`` 记法），模板里的 ``{字段}``
    则由 ``values`` 填充。声明了 select 却选不到模板时**必须显式失败**：
...
#: 检索经 RAG Skill 走，规范化（丢弃不可定位命中）由 Skill 负责。
_EVIDENCE_BINDING = {
...
__all__ = ["ACTION_BINDINGS", "ACTION_CAPABILITIES", "CORRECT_PROMPT_TEMPLATE", "TEACH_PROMPT_TEMPLATE"]